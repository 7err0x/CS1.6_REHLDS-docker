...
| [`image/mapcycle.txt`](image/mapcycle.txt) | **Respawn** map rotation (`mapcyclefile`). Keep in sync with [`maps-respawn.ini`](image/zombiemod/maps-respawn.ini) (end-of-map **vote** pool). |
...
| [`image/zombiemod/maps-respawn.ini`](image/zombiemod/maps-respawn.ini) | **Respawn** mapchooser vote pool (baked as **`maps.ini`**). Mirrors **`mapcycle.txt`**. |
| [`image/zombiemod/maps-biohazard.ini`](image/zombiemod/maps-biohazard.ini) | **Biohazard** mapchooser vote pool (baked as **`maps.ini`**). |
| [`image/mapcycle.biohazard.txt`](image/mapcycle.biohazard.txt) | **Biohazard** rotation (`mapcyclefile`). **`BIOHAZARD_START_MAP`** defaults to **`cs_estate`**. |
...
1. **End-of-map vote** — **`cs16_mapchooser.amxx`**: polls every **15 seconds** and opens the vote once end-of-map conditions are met (see below). Candidate maps come from **`addons/amxmodx/configs/maps.ini`** when baked for the profile (**respawn:** [`maps-respawn.ini`](image/zombiemod/maps-respawn.ini); **biohazard:** [`maps-biohazard.ini`](image/zombiemod/maps-biohazard.ini)); otherwise from **`mapcyclefile`**. Each vote shows **5 random maps** from that pool, plus **Extend current map** and **None** when timelimit mode allows extension. The menu stays open **`amx_vote_time`** seconds (baked **`45`** in [`image/zombiemod/amxx.cfg`](image/zombiemod/amxx.cfg); **`rcon amx_vote_time 60`** to change live).
...
- **`TEX_InitFromWad: couldn't open de_vegas.wad`:** **`zm_toronto`** (and some fy_/aim maps) need **`de_vegas.wad`**. Run **`docker compose --profile download-assets run --rm download-game-assets`** (auto-fetches into **`./data/cs16-game-assets/wads/`**), then restart **`cs16`** / **`cs16-biohazard`** so **`cs16-entrypoint`** copies it to **`cstrike/de_vegas.wad`**. Or copy from a full CS install into **`wads/`** manually.
