...
# ReUnion (Metamod only — no AMXX): non-Steam / mixed clients need auth emulation; AMXX+ReAPI still crashes on map load here.
...
# Only ReUnion in Metamod — do not load amxmodx_mm (see README).
RUN printf '%s\n' 'linux addons/reunion/reunion_mm_i386.so' > /opt/steam/hlds/cstrike/addons/metamod/plugins.ini
...
RUN chown -R steam:steam /opt/steam/hlds/cstrike/addons/reunion /opt/steam/hlds/cstrike/addons/metamod/plugins.ini \
